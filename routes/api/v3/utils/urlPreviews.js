...
async function getURLPreview(url){
    ...
    let responseHTML = `
        <div style='max-width: 300px; border: 1px; padding: 3px; text-align: center;'>
            <a href=${escapeHTML(urlContent)}>
                <p><strong>
                    ${escapeHTML(titleContent)}
                </strong></p>`
    if (imgSrc !== undefined) {
        responseHTML += `<img src=${escapeHTML(imgSrc)} style='max-height: 200px; max-width: 270px;'/>`
    }
    responseHTML += `</a>`
    if (descContent !== undefined) {
        responseHTML += `<p>${escapeHTML(descContent)}</p>`
    }
    if(videoContent !== undefined) {
        responseHTML += `<p>This is the link to the video: ${escapeHTML(videoContent)}</p>`
    }
    responseHTML += `</div>`
